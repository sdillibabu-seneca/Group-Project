...
def nmap_scan():
    ...
    nmap_box = scrolledtext.ScrolledText(nmap_frame, bg='white', relief=GROOVE, height=12, width=10, font='TkFixedFont', state=DISABLED)
    nmap_box.pack(side=BOTTOM)
    ...
    submit_nmap_scan_yes_button.place(relx=0.5,rely=0.5)
    submit_nmap_scan_no_button=Button(nmap_frame,text='No', command= lambda:enter_victim_ip())
    submit_nmap_scan_no_button.place(relx=0.10,rely=0.5)
...
def check_var(values, required_var_list):
    ...
    check_var_frame = Frame(root, padx=20, pady=20,bg='grey')
    check_var_frame.place(relx=0,rely=0.1,relheight=0.4,relwidth=0.98)
    label5=Label(check_var_frame,text='Are all of these variables correct?')
    label5.place(relx=0.1,rely=0.3)
    
    check_var_box = scrolledtext.ScrolledText(check_var_frame, bg='white', relief=GROOVE, height=12, width=10, font='TkFixedFont', state=DISABLED)
    ...
    submit_check_var_yes_button=Button(check_var_frame,text='Yes', command= lambda:attack_output())
    submit_check_var_yes_button.place(relx=0.5,rely=0.5)
    submit_check_var_no_button=Button(check_var_frame,text='No', command= lambda:change_var(required_var_list))
    submit_check_var_no_button.place(relx=0.10,rely=0.5)
            
def change_var(required_var_list):
    change_var_frame = Frame(root, padx=20, pady=20, bg='grey')
    change_var_frame.place(relx=0, rely=0.1, relheight=0.4, relwidth=0.98)

    change_var_label = Label(change_var_frame, text='Please fill the below fields, choose a parameter ')
    ...
    variable_selection_button = Button(change_var_frame, text='Submit', command=lambda:validate_variable_selection(variable_list,required_var_list))
...
def attack_output():
    ...
    attack_output_box = scrolledtext.ScrolledText(attack_output_frame, bg='white', relief=GROOVE, height=12, width=20, font='TkFixedFont', state=DISABLED) #12 10
    attack_output_box.pack(side=BOTTOM)

    attack_output_box.configure(state=NORMAL)
    output_str = 'check the terminal'
    attack_output_box.insert(END, output_str) # insert the string into the text box
    #attack_output_box.insert(END, sys.stdout.getvalue())
    #attack_output_box.insert(END,sys.stdout.write) 
    attack_output_box.configure(state=DISABLED) # 192.168.157.206
